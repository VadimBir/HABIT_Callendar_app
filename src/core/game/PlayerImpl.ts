...
export class PlayerImpl implements Player {
  ...
  private _growthRatio: [number, number, number] = [1 / 3, 1 / 3, 1 / 3];
  private _targetRatio: [number, number, number] = [1 / 3, 1 / 3, 1 / 3];
  // --- end Stream B ---
  ...
  // --- Stream A: autopilot ---
  private _autopilotEnabled = false;
  // --- end Stream A: autopilot ---
  ...
  constructor(
    ...
    // --- Stream B: 3 troop types ---
    // Distribute starting troops evenly across the 3 slots, remainder to slot 0.
    const startInt = toInt(startTroops);
    const per = startInt / 3n;
    const remainder = startInt - per * 3n;
    this._troopsByType = [per + remainder, per, per];
    // --- end Stream B ---
  ...
  toUpdate(
    ...
    if (
      ...
      (prev.tilesOwned !== full.tilesOwned ||
        ...
        // --- Stream B: 3 troop types ---
        prev.troopsByType === undefined ||
        full.troopsByType === undefined ||
        prev.troopsByType[0] !== full.troopsByType[0] ||
        prev.troopsByType[1] !== full.troopsByType[1] ||
        prev.troopsByType[2] !== full.troopsByType[2])
      // --- end Stream B ---
    ) {
      // --- Stream B: 3 troop types ---
      // Septuple: [smallID, tilesOwned, gold, troops, t0, t1, t2]. troopsByType
      // is stat churn carried on the packed channel alongside the other stats.
      const tbt = full.troopsByType ?? [0, 0, 0];
      statsOut.push(
        full.smallID!,
        full.tilesOwned!,
        Number(full.gold),
  ...
  private toFullUpdate(): PlayerUpdate {
    ...
    return {
      ...
      troops: this.troops(),
      // --- Stream B: 3 troop types ---
      troopsByType: this.troopsByType(),
      // --- end Stream B ---
      allies: allies,
      embargoes: embargoes,
  ...
  setTroops(troops: number) {
    // --- Stream B: 3 troop types ---
    // Set the total while preserving the current distribution proportions.
    const target = toInt(troops);
    if (target <= 0n) {
      this._troopsByType = [0n, 0n, 0n];
      return;
    }
    const current = this._troopsByType[0] + this._troopsByType[1] + this._troopsByType[2];
    if (current <= 0n) {
      // No existing distribution; fall back to growth ratio.
      this._troopsByType = [0n, 0n, 0n];
      this.addTroops(troops);
      return;
    }
    let assigned = 0n;
    const next: [bigint, bigint, bigint] = [0n, 0n, 0n];
    for (let i = 1; i < 3; i++) {
      next[i] = (this._troopsByType[i] * target) / current;
      assigned += next[i];
    }
    next[0] = target - assigned;
    if (next[0] < 0n) next[0] = 0n;
    this._troopsByType = next;
    // --- end Stream B ---
  }
  ...
  troops(): number {
    // --- Stream B: 3 troop types ---
    return Number(
      this._troopsByType[0] + this._troopsByType[1] + this._troopsByType[2],
    );
    // --- end Stream B ---
  }

  // --- Stream B: 3 troop types ---
  troopsByType(): [number, number, number] {
    return [
      Number(this._troopsByType[0]),
      Number(this._troopsByType[1]),
      Number(this._troopsByType[2]),
    ];
  }

  growthRatio(): [number, number, number] {
    return [...this._growthRatio];
  }

  targetRatio(): [number, number, number] {
    return [...this._targetRatio];
  }

  setGrowthRatio(r0: number, r1: number): void {
    let a = within(r0, 0, 1);
    let b = within(r1, 0, 1);
    if (a + b > 1) {
      // Clamp so r0 + r1 <= 1, scaling proportionally.
      const sum = a + b;
      a = a / sum;
      b = b / sum;
    }
    this._growthRatio = [a, b, 1 - a - b];
  }

  setTargetRatio(r0: number, r1: number): void {
    let a = within(r0, 0, 1);
    let b = within(r1, 0, 1);
    if (a + b > 1) {
      const sum = a + b;
    ...
    this._targetRatio = [a, b, 1 - a - b];
  }
  // --- end Stream B ---

  addTroops(troops: number, type?: 0 | 1 | 2): void {
    if (troops < 0) {
      this.removeTroops(-1 * troops);
      return;
    }
    // --- Stream B: 3 troop types ---
    const toAdd = toInt(troops);
    if (toAdd <= 0n) return;
    if (type !== undefined) {
      this._troopsByType[type] += toAdd;
      return;
    }
    // Distribute by growth ratio (bigint math, remainder to slot 0).
    let assigned = 0n;
    const add: [bigint, bigint, bigint] = [0n, 0n, 0n];
    for (let i = 1; i < 3; i++) {
      add[i] = (toAdd * BigInt(Math.round(this._growthRatio[i] * 1_000_000))) / 1_000_000n;
      assigned += add[i];
    }
    add[0] = toAdd - assigned;
    if (add[0] < 0n) add[0] = 0n;
    this._troopsByType[0] += add[0];
    this._troopsByType[1] += add[1];
    this._troopsByType[2] += add[2];
    // --- end Stream B ---
  }
  removeTroops(troops: number): number {
    ...
    const current =
      this._troopsByType[0] + this._troopsByType[1] + this._troopsByType[2];
    const toRemove = minInt(current, toInt(troops));
    if (toRemove <= 0n) return 0;
    if (toRemove >= current) {
      this._troopsByType = [0n, 0n, 0n];
      return Number(current);
    }
    // Proportional removal across slots; remainder taken from slot 0.
    let removed = 0n;
    const rem: [bigint, bigint, bigint] = [0n, 0n, 0n];
    for (let i = 1; i < 3; i++) {
      rem[i] = (this._troopsByType[i] * toRemove) / current;
      removed += rem[i];
    }
    rem[0] = toRemove - removed;
    for (let i = 0; i < 3; i++) {
      let next = this._troopsByType[i] - rem[i];
      if (next < 0n) next = 0n;
      this._troopsByType[i] = next;
    }
  ...
  // --- Stream B: 3 troop types ---
  // Move the current distribution toward _targetRatio by at most `rate`
  // troops total, taking proportionally from over-represented slots and
  // giving to under-represented slots. Net total is unchanged.
  applySlowConversion(rate: number): void {
    if (rate <= 0) return;
    const total =
      this._troopsByType[0] + this._troopsByType[1] + this._troopsByType[2];
    if (total <= 0n) return;
    const budget = toInt(rate);
    if (budget <= 0n) return;

    // Desired count per slot.
    const desired: bigint[] = [0n, 0n, 0n];
    let assigned = 0n;
    for (let i = 1; i < 3; i++) {
      desired[i] =
        (total * BigInt(Math.round(this._targetRatio[i] * 1_000_000))) /
        1_000_000n;
      assigned += desired[i];
    }
    desired[0] = total - assigned;

    // Surplus (over target) and deficit (under target) per slot.
    const surplus: bigint[] = [0n, 0n, 0n];
    const deficit: bigint[] = [0n, 0n, 0n];
    let totalSurplus = 0n;
    let totalDeficit = 0n;
    for (let i = 0; i < 3; i++) {
      const diff = this._troopsByType[i] - desired[i];
      if (diff > 0n) {
        surplus[i] = diff;
        totalSurplus += diff;
      } else if (diff < 0n) {
    ...
    // Amount we can actually move this tick.
    let move = minInt(budget, minInt(totalSurplus, totalDeficit));
    if (move <= 0n) return;

    // Take proportionally from over-represented slots.
    const taken: bigint[] = [0n, 0n, 0n];
    let takenTotal = 0n;
    for (let i = 0; i < 3; i++) {
      if (surplus[i] <= 0n) continue;
      taken[i] = (surplus[i] * move) / totalSurplus;
      takenTotal += taken[i];
    }
    // Assign rounding remainder to the largest-surplus slot.
    let remainder = move - takenTotal;
    if (remainder > 0n) {
      let maxIdx = 0;
      for (let i = 1; i < 3; i++) {
        if (surplus[i] > surplus[maxIdx]) maxIdx = i;
      }
      taken[maxIdx] += remainder;
    }
    for (let i = 0; i < 3; i++) {
      this._troopsByType[i] -= taken[i];
      if (this._troopsByType[i] < 0n) this._troopsByType[i] = 0n;
    }
    // Recompute exactly how much was taken (after clamping) and redistribute.
    const actuallyMoved = taken[0] + taken[1] + taken[2];

    // Give proportionally to under-represented slots.
    let givenTotal = 0n;
    const given: bigint[] = [0n, 0n, 0n];
    for (let i = 0; i < 3; i++) {
      if (deficit[i] <= 0n) continue;
      given[i] = (deficit[i] * actuallyMoved) / totalDeficit;
      givenTotal += given[i];
    }
    let giveRemainder = actuallyMoved - givenTotal;
    if (giveRemainder > 0n) {
      let maxIdx = 0;
      for (let i = 1; i < 3; i++) {
        if (deficit[i] > deficit[maxIdx]) maxIdx = i;
      }
      given[maxIdx] += giveRemainder;
    }
    for (let i = 0; i < 3; i++) {
      this._troopsByType[i] += given[i];
    }
  }
  // --- end Stream B ---
