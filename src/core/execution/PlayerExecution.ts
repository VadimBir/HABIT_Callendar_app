...
export class PlayerExecution implements Execution {
  ...
  tick(ticks: number) {
    ...
    // When the army is at (near) capacity, slowly re-allocate the existing
    // distribution toward the player's target ratio.
    if (
      this.player.troops() >=
      this.config.maxTroops(this.player) * 0.99
    ) {
      this.player.applySlowConversion(
        this.config.troopConversionRate(this.player),
      );
    }
    // --- end Stream B ---
