...
// --- Stream A: autopilot ---
import { ToggleAutopilotEvent } from "../../Transport";
// --- end Stream A: autopilot ---
// --- Stream B: troop composition ---
import { SetTroopRatioEvent } from "../../Transport";
// --- end Stream B: troop composition ---
...
export class ControlPanel extends LitElement implements Controller {
  ...
  // --- Stream B: troop composition ---
  @state()
  private _troopsByType: [number, number, number] = [0, 0, 0];
  // T1 and T2 growth shares as percentages (0-100); T3 = remainder.
  // These drive both where new growth goes and the slow re-allocation target.
  @state()
  private _growthT1 = 34;
  @state()
  private _growthT2 = 33;
  private _troopRatioDebounce: ReturnType<typeof setTimeout> | null = null;
  // --- end Stream B: troop composition ---
  ...
  tick() {
    ...
    // --- Stream B: troop composition ---
    this._troopsByType = player.troopsByType();
    // --- end Stream B: troop composition ---
  ...
  private toggleAutopilot() {
    ...
    this.eventBus.emit(new ToggleAutopilotEvent(this._autopilotEnabled));
    this.requestUpdate();
  }

  private renderAutopilotToggle() {
    return html`
      <div class="flex justify-end mt-1" translate="no">
        <button
          @click=${() => this.toggleAutopilot()}
          class="px-2 py-0.5 rounded-md border text-xs font-bold ${this
            ._autopilotEnabled
            ? "border-green-400 text-green-400"
            : "border-gray-600 text-white"}"
        >
          Autopilot ${this._autopilotEnabled ? "ON" : "OFF"}
        </button>
      </div>
    `;
  }
  ...
  // --- Stream B: troop composition ---
  // Keep T1 + T2 <= 100 (T3 takes the remainder), then emit the ratios.
  private onGrowthSlider(which: 0 | 1, e: Event) {
    const value = Number((e.target as HTMLInputElement).value);
    if (which === 0) {
      this._growthT1 = value;
      if (this._growthT1 + this._growthT2 > 100) {
        this._growthT2 = 100 - this._growthT1;
      }
    } else {
      this._growthT2 = value;
      if (this._growthT1 + this._growthT2 > 100) {
        this._growthT1 = 100 - this._growthT2;
      }
    }
    this.emitTroopRatioDebounced();
  }

  private emitTroopRatioDebounced() {
    if (this._troopRatioDebounce !== null) {
      clearTimeout(this._troopRatioDebounce);
    }
    this._troopRatioDebounce = setTimeout(() => {
      const r0 = this._growthT1 / 100;
      const r1 = this._growthT2 / 100;
      // Growth and target use the same player-chosen mix; conversion toward it
      // happens slowly (0.3x growth) once the army is at max.
      this.eventBus.emit(new SetTroopRatioEvent(r0, r1, r0, r1));
    }, 200);
  }

  private renderTroopComposition() {
    const total = Math.max(
      this._troopsByType[0] + this._troopsByType[1] + this._troopsByType[2],
      1,
    );
    const pct = (n: number) => (n / total) * 100;
    const t3 = Math.max(0, 100 - this._growthT1 - this._growthT2);
    return html`
      <div class="mt-1 px-1" translate="no">
        <div class="flex justify-between text-[10px] text-gray-300 mb-0.5">
          <span>Troop mix</span>
          <span>
            <span class="text-blue-400">T1 ${this._growthT1}%</span>
            ·
            <span class="text-red-400">T2 ${this._growthT2}%</span>
            ·
            <span class="text-green-400">T3 ${t3}%</span>
          </span>
        </div>
        <!-- live composition of current army -->
        <div class="flex h-1.5 w-full rounded overflow-hidden mb-1">
          <div
            class="bg-blue-500 h-full"
            style="width: ${pct(this._troopsByType[0])}%"
          ></div>
          <div
            class="bg-red-500 h-full"
            style="width: ${pct(this._troopsByType[1])}%"
          ></div>
          <div
            class="bg-green-500 h-full"
            style="width: ${pct(this._troopsByType[2])}%"
          ></div>
        </div>
        <input
          type="range"
          min="0"
          max="100"
          .value=${String(this._growthT1)}
          @input=${(e: Event) => this.onGrowthSlider(0, e)}
          class="w-full accent-blue-500"
          aria-label="Type 1 growth share"
        />
        <input
          type="range"
          min="0"
          max="100"
          .value=${String(this._growthT2)}
          @input=${(e: Event) => this.onGrowthSlider(1, e)}
          class="w-full accent-red-500"
          aria-label="Type 2 growth share"
        />
      </div>
    `;
  }
  ...
  render() {
    return html`
      ...
      >
        ...
        <!-- Stream A: autopilot toggle (isolated div to avoid colliding with stream-b's slider section) -->
        ${this.renderAutopilotToggle()}
        <!-- end Stream A: autopilot toggle -->
        <!-- Stream B: troop composition sliders -->
        ${this.renderTroopComposition()}
        <!-- end Stream B: troop composition sliders -->
      </div>
    `;
  }
